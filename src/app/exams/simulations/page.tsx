...
const pageTitle = "ชุดจำลองสนามครูผู้ช่วยทั้งหมด";
const pageDescription = "รวมชุดจำลองสนามครูผู้ช่วยตามสังกัด เอก และปีสอบ พร้อมเลือกทำภาค ก ภาค ข และภาค ค";

export const metadata: Metadata = {
  title: pageTitle,
  description: pageDescription,
  alternates: {
    canonical: "/exams/simulations",
  },
  openGraph: {
    title: pageTitle,
    description: pageDescription,
    url: "/exams/simulations",
    ...
    <main className="min-h-screen bg-[#f7f8fc] text-slate-950">
      ...
      <section className="bg-[#071f4a] text-white">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <div className="mt-5 grid gap-6 lg:grid-cols-[1fr_360px] lg:items-end">
            <div>
              <p className="text-sm font-black text-white/70">ชุดจำลองสนาม</p>
              <h1 className="mt-2 text-4xl font-black leading-tight sm:text-5xl">เลือกชุดจำลองตามสังกัดและเอก</h1>
              <p className="mt-4 max-w-2xl text-lg font-semibold leading-8 text-white/85">
                เลือกชุดสอบตามสนามจริง แล้วเข้าไปเลือกทำ ภาค ก ภาค ข หรือ ภาค ค ภายในชุดนั้น
              </p>
            </div>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="rounded-lg border border-white/15 bg-white/12 p-4 backdrop-blur">
                ...
                <p className="mt-1 text-3xl font-black text-[#ffd35a]">{packages.length}</p>
              </div>
              <div className="rounded-lg border border-white/15 bg-white/12 p-4 backdrop-blur">
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-black text-[#0b66c3]">รายการชุดจำลอง</p>
            <h2 className="mt-1 text-2xl font-black text-[#071f4a]">เลือกสนาม เอก และปีสอบ</h2>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1">
            {["ทั้งหมด", ...affiliations.map((item) => item.label), "ภาค ก", "ภาค ข", "ภาค ค"].map((item) => (
              <button
                key={item}
